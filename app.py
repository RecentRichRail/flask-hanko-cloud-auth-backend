...
for jwk in jwks_data["keys"]: 
    kid = jwk["kid"]
    public_keys[kid] = jwt_algorithms.RSAAlgorithm.from_jwk(jwk)
...
def auth():
    ...
    logging.info(f"Checking for Authorization header...")
    auth_header = request.headers.get('Authorization')
    if not auth_header or not auth_header.startswith('Bearer '):
        logging.info(f"No Authorization header found. Redirecting to /login...")
        return jsonify({"message": "Not authorized"}), 401
    jwt_token = auth_header.split(' ', 1)[1].strip()
    try:
        logging.info(f"Authorization header found. Verifying...")
